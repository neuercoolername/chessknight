...
export default function Board() {
  ...
  const getKnightMoves = () => {
    ...
    const moves = [
      { col: String.fromCharCode(col.charCodeAt() + 2), row: parseInt(row, 10) + 1 },
      { col: String.fromCharCode(col.charCodeAt() + 2), row: parseInt(row, 10) - 1 },
      { col: String.fromCharCode(col.charCodeAt() - 2), row: parseInt(row, 10) + 1 },
      { col: String.fromCharCode(col.charCodeAt() - 2), row: parseInt(row, 10) - 1 },
      { col: String.fromCharCode(col.charCodeAt() + 1), row: parseInt(row, 10) + 2 },
      { col: String.fromCharCode(col.charCodeAt() + 1), row: parseInt(row, 10) - 2 },
      { col: String.fromCharCode(col.charCodeAt() - 1), row: parseInt(row, 10) + 2 },
      { col: String.fromCharCode(col.charCodeAt() - 1), row: parseInt(row, 10) - 2 }
    ];
    return moves.filter(({ col, row }) => cols.includes(col) && rows.includes(row.toString()));
  };
...
//   console.log(getKnightMoves())
  ...
  const isKnightMove = (row, col) => {
    ...
    return knightMoves.some(({ col: moveCol, row: moveRow }) => moveCol === col && moveRow.toString() === row);
  };
